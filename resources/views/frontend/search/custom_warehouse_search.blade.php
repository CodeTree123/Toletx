...
<div class="col-lg-8 col-md-8 col-sm-12 col-12 slider-center slide-show ">
  <div class="container">
    <a class="col-small d-flex flex-column justify-content-center align-items-center me-2 p-3 mb-2 text-center " hrefe="{{route('warehouse')}}" style="width:150px;">
  ...
  <div class="items">
    ...
    <div class="card mx-2 slick-slider-card" style="width: 12rem">
      <img src="{{ asset('public/uploads/warehouses') }}/{{ $arrays->photo }}" class="card-img-top" alt="..." />
      <div class="card-body main-slider-body main-slider-body text-center px-1">
        ...
        <h5 class="card-title main-slider text-capitalize mb-3">warehouse Rent : {{ $arrays->price }} Tk</h5>
        <p class=" main-slider">
          <span>Available form: {{ Carbon\Carbon::parse($arrays->date)->format('d/m/Y') }}</span>
        </p>
        <a href="{{ url('/warehouse/details/') }}/{{ $arrays->id }}" class="btn btn btn-custom-color2 check-detail mt-2  ">Check Details</a>
      </div>
    </div>
    @else
    <div class="card mx-2 slick-slider-card" style="width: 12rem">

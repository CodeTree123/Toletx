...
<div class="col-lg-8 col-md-8 col-sm-12 col-12 slider-center slide-show ">
  <div class="container">
    <a class="col-small d-flex flex-column justify-content-center align-items-center me-2 p-3 mb-2 text-center " href="{{route('pond')}}" style="width:150px;">
  ...
  <div class="items">
    ...
    <div class="card mx-2 slick-slider-card" style="width: 12rem">
      <img src="{{ asset('public/uploads/ponds') }}/{{ $arrays->photo }}" class="card-img-top" alt="..." />
      <div class="card-body main-slider-body main-slider-body px-1">
        <p class="card-text main-slider">{{ Str::limit($arrays->title,42) }}</p>
        <h5 class="card-title main-slider text-capitalize mb-3">Pond Rent : {{ $arrays->price }} Tk</h5>
        <p class=" main-slider">
          <span>Available form: {{ Carbon\Carbon::parse($arrays->date)->format('d/m/Y') }}</span>
        </p>

        <a href="{{ url('/pond/details/') }}/{{ $arrays->id }}" class="btn btn-custom-color2 check-detail mt-2  ">Check Details</a>
